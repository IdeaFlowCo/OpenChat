 ...
 * (openchat-bbr). Extracted from McpSetupCard's snippetFor('chatgpt', key)
 * so it can be reused by the one-click "Copy agent setup" action in Settings.
 *
 * The returned text is a self-contained prose blob you can paste into any
 * chatbot (ChatGPT, Claude, Gemini, any LLM that can make HTTPS requests).
 * No MCP server, no shell, no file writes, no external guide required —
 * the key is inlined and the REST cheatsheet is included.
 *
 * Keep this identical to the web client's copy at
 * OpenChat/client/src/utils/agentSetupBlob.ts.
 */
