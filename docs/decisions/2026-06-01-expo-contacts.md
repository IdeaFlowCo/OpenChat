...
- **User search + conversation creation** in /m/, /d/, native (`GET /api/chat/search`, then `POST /api/chat/conversations` with `participantIds`) — members can find default-discoverable accounts by display name or use exact email for accounts that allow it
- **Contact picker** over `/api/chat/contacts` — supports the same per-profile discovery rules and never returns email addresses
