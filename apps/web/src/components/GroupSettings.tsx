...
export function GroupSettings({ open, onClose, conversation }: GroupSettingsProps) {
  ...
  useEffect(() => {
    if (!showAdd) return;
    if (debouncedSearch.length === 0 && !currentUser?.openUserDirectoryEnabled) {
      setResults([]);
      return;
    }
    let cancelled = false;
  ...
  }, [debouncedSearch, showAdd, searchContacts, conversation.participants, currentUser?.openUserDirectoryEnabled]);
          ...
          <section>
            ...
            {showAdd && (
              <div className="mb-3 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg">
                ...
                {(search.length > 0 || currentUser?.openUserDirectoryEnabled) && (
