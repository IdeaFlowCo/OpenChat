...
export const openapiSpec = {
  ...
  paths: {
    ...
    '/api/chat/contacts': {
      get: { operationId: 'listContacts', tags: ['Chat'], summary: 'Find a user for starting a conversation', description: 'When OPENCHAT_OPEN_USER_DIRECTORY is enabled, an empty query pages through default-discoverable accounts. Otherwise, find accounts by partial display name or name/email-only accounts by complete exact email. Target profile settings are honored, and email addresses are never returned.', parameters: [{ name: 'q', in: 'query', schema: { type: 'string' }, description: "Display name, complete email address, or 'self'/'me'/'myself'" }, { name: 'limit', in: 'query', schema: { type: 'integer', minimum: 1, maximum: 100, default: 50 } }, { name: 'offset', in: 'query', schema: { type: 'integer', minimum: 0, default: 0 } }], responses: { '200': ok({ type: 'array', items: { type: 'object' } }) } },
    },
    '/api/chat/search': {
